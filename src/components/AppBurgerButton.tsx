...
export function AppBurgerButton({ onClick, className = '' }: AppBurgerButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex min-h-16 w-[60px] shrink-0 items-center justify-center rounded-xl border border-[#e4e4e4] bg-white text-[#1e1e1f] shadow-sm transition-colors hover:bg-[#f5f5f5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1e1e1f]/20 ${className}`.trim()}
